"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var fs_extra_1 = require("fs-extra");
var node_path_1 = require("node:path");
var command_line_args_1 = require("command-line-args");
var command_line_usage_1 = require("command-line-usage");
var node_url_1 = require("node:url");
var lib_js_1 = require("./lib.js");
// ref. https://zenn.dev/risu729/articles/dirname-in-esm
function my_dirname() {
    if ('dirname' in import.meta) {
        return import.meta.dirname;
    }
    else {
        var filename = (0, node_url_1.fileURLToPath)(import.meta.url);
        return node_path_1.default.dirname(filename);
    }
}
;
(function main() {
    var _a;
    return __awaiter(this, void 0, void 0, function () {
        var optionDef, options, noargs, usage, _b, inputDir, tempDir, configMaker;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    optionDef = [
                        { name: 'input', alias: 'i', type: String, description: 'input folder including configuration templates' },
                        { name: 'output', alias: 'o', type: String, description: 'output folder including #curr/#prev/#stage' },
                        { name: 'help', alias: 'h', type: Boolean, description: 'Display this usage guide' }
                    ];
                    options = (0, command_line_args_1.default)(optionDef);
                    noargs = options.input === undefined || !((_a = fs_extra_1.default.statSync(options.input, { throwIfNoEntry: false })) === null || _a === void 0 ? void 0 : _a.isDirectory()) || options.output === undefined;
                    if (options.help || noargs) {
                        if (noargs) {
                            console.log('input and output are mandatory options and input directory must exist');
                        }
                        usage = (0, command_line_usage_1.default)([
                            {
                                header: 'Config Maker',
                                content: 'generatates configuration files from templates, compare with current files and manage them under git control'
                            },
                            {
                                header: 'Synopsis',
                                content: [
                                    '> node config_maker.js -input \\#conf -output \\#work',
                                ]
                            },
                            {
                                header: 'Options',
                                optionList: optionDef
                            },
                            {
                                header: 'Folders',
                                content: [
                                    'All the follwing folders are version-controled by git.',
                                    'work/#prev: The previous state for files generated from templates.',
                                    'work/#curr: The current state for files generated from templates.',
                                    'work/#stage: Staging folder includes hardlinks to actual configuraiton files.'
                                ]
                            }
                        ]);
                        console.log(usage);
                        return [2 /*return*/];
                    }
                    _b = [options.input, options.output].map(function (v) { return node_path_1.default.join(my_dirname(), v); }), inputDir = _b[0], tempDir = _b[1];
                    configMaker = new lib_js_1.default(inputDir, tempDir);
                    return [4 /*yield*/, configMaker.collect()];
                case 1:
                    _c.sent();
                    console.log(configMaker.specs);
                    return [4 /*yield*/, configMaker.prepare()];
                case 2:
                    _c.sent();
                    return [4 /*yield*/, configMaker.generate()];
                case 3:
                    _c.sent();
                    return [4 /*yield*/, configMaker.update()];
                case 4:
                    _c.sent();
                    return [4 /*yield*/, configMaker.install()
                        // rollback() or commit()
                    ];
                case 5:
                    _c.sent();
                    return [2 /*return*/];
            }
        });
    });
})(); // invoking main
